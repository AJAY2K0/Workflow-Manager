...
import InProgress from "../../Dashboard/Admin/FilteredTask/InProgress";

const ShowNewTask = ({ data, onClose }) => {
  const validTasks = data.tasks.filter((t) => t.inProgress);
          ...
          {!hasNoTasks &&
            data.tasks.map((elem, index) => {
              const Wrapper = ({ children }) => (
                <div className="w-full sm:w-[48%] lg:min-w-[400px] lg:max-w-[400px]">
                  {children}
                </div>
              );
